  ...
  if (isMonthly) {
    for (const c of p.contributions) {
      ...
      items.push({
        ...
        name: (
          <Link href={`/bills/${c.groupId ?? c.householdId}/expenses/${c.billId}`} className="text-ink no-underline" onClick={(e) => e.stopPropagation()}>
            {c.billTitle}
    ...
    for (const c of splitMap.values()) {
      items.push({
        ...
        dayLabel: "—",
        name: (
          <Link href={`/bills/${c.groupId ?? c.householdId}/expenses/${c.billId}`} className="text-ink no-underline">
            {c.billTitle}
            <span className="font-normal text-base text-ink-3 ml-3">{c.householdName}</span>
          </Link>
        ),
