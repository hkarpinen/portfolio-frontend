...
export function MarkPaidButton({ householdId, householdExpenseId, isPaid = false, occurrenceDate }: Props) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={isPending}
      className="inline-flex items-center gap-3 py-[6px] px-[14px] rounded-full text-base font-medium" style={{ background: isPending ? "var(--paper-3)" : "var(--success-s)", color: isPending ? "var(--text-3)" : "var(--success)", border: "1px solid oklch(68% 0.18 152 / 0.25)", cursor: isPending ? "not-allowed" : "pointer", transition: "background 110ms", opacity: isPending ? 0.7 : 1 }}
    >
      {isPending ? (
