...
export default function RegisterPage() {
  ...
  async function onSubmit(data: RegisterInput) {
    ...
    try {
      ...
      await api.post("/api/identity/register", { email: data.email, displayName: data.displayName, password: data.password, captchaToken });
      setSubmitted(true);
    } catch (err) {
  ...
  if (submitted) {
    return (
      <div className="ed-auth-card">
        <h1 className="ed-h1">Check your <em>email</em></h1>
  ...
  return (
    <>
      <div className="ed-auth-card">
        <h1 className="ed-h1">Create an <em>account</em></h1>
        <p className="ed-hint mt-2 mb-8">
          Already have one?{" "}
          <Link href="/login" className="text-red font-semibold">Sign in →</Link>
        </p>

        {/* OAuth */}
        <div className="grid grid-cols-1 gap-3 mb-8">
          <Btn variant="secondary" fullWidth iconLeft={GITHUB_SVG} onClick={() => { window.location.href = "/api/identity/oauth/github"; }}>
            Continue with GitHub
          </Btn>
          <Btn variant="secondary" fullWidth iconLeft={GOOGLE_SVG} onClick={() => { window.location.href = "/api/identity/oauth/google"; }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
          ...
          <Input label="Display name" type="text" placeholder="What should we call you?" autoComplete="name" error={errors.displayName?.message} {...register("displayName")} />
          <Input label="Email address" type="email" placeholder="you@email.com" autoComplete="email" error={errors.email?.message} {...register("email")} />
          ...
          <div>
            <div className="relative">
              ...
              />
              <button type="button" onClick={() => setShowPw(s => !s)} aria-label={showPw ? "Hide password" : "Show password"} aria-pressed={showPw} className="absolute right-0 bottom-[7px] bg-transparent cursor-pointer text-ink-3 p-0 border-none leading-none">
                {EYE(showPw)}
              </button>
            </div>
          ...
          <div className="relative">
            ...
            />
            <button type="button" onClick={() => setShowConfirm(s => !s)} aria-label={showConfirm ? "Hide password" : "Show password"} aria-pressed={showConfirm} className="absolute right-0 bottom-[7px] bg-transparent cursor-pointer text-ink-3 p-0 border-none leading-none">
              {EYE(showConfirm)}
            </button>
          </div>
          ...
          <p className="ed-label-muted leading-relaxed">
            ...
            <a href="/terms" className="underline hover:text-red">terms of service</a>
            {" "}and{" "}
            <a href="/privacy" className="underline hover:text-red">privacy policy</a>.
          </p>
