...
export function ItemTableRow({ item, isLast }: { item: TableItem; isLast: boolean }) {
  ...
  return (
    <tr>
      ...
      <td
        className={`whitespace-nowrap text-right font-serif font-bold pr-6${borderCls}`}
        style={{ ...tdBase }}
      >
        {item.count !== undefined && item.count > 1 ? (
          <>
            <span className="mr-[5px] font-mono text-sm font-normal text-ink-3">
              {item.currency}
            </span>
            {item.amount} <span className="text-sm font-medium text-ink-3">×{item.count}</span>
          </>
        ) : (
          <>
            <span className="mr-[5px] font-mono text-sm font-normal text-ink-3">
      ...
      {item.payToggle !== undefined && (
        <td
          className={`w-[110px] pr-8 text-right whitespace-nowrap${borderCls}`}
          style={{ ...tdBase }}
        >
          {item.payToggle}
        </td>
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-[3px] flex flex-wrap items-center gap-3">
          ...
          {/* bg/color/border are data-driven (Shared vs Personal) */}
          <span
            className={`inline-block px-3 py-0 font-mono text-sm ${item.type === "Shared" ? "bg-red-soft text-red [border:1px_solid_rgba(178,42,26,0.3)]" : "border border-ink-3 bg-paper-3 text-ink-3"}`}
          >
            {item.type}
          </span>
          {item.count !== undefined && item.count > 1 && (
      ...
      <div className="flex shrink-0 flex-col items-end gap-[5px]">
        <span className="whitespace-nowrap font-serif text-base font-bold text-ink">
          <span className="mr-[5px] font-mono text-sm font-normal text-ink-3">{item.currency}</span>
          {item.amount}
        </span>
        {item.payToggle}
      </div>
