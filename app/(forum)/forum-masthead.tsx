...
function actionFor(pathname: string, slug?: string): React.ReactNode {
  ...
  if (slug) {
    return (
      <Btn href={`/forum/g/${slug}/threads/new`} variant="primary" size="sm"
  ...
  if (pathname === "/forum" || pathname === "/" || pathname === "/forum/") {
    return (
      <Btn href="/forum/new" variant="primary" size="sm"
           iconLeft={<Icon name="plus" size={12} strokeWidth={2.5} />}>
        New community
      </Btn>
    );
...
export function ForumMasthead() {
  ...
  return (
    <MastheadRow desk={desk} action={actionFor(pathname, slug)} />
  );
}
